...
Phone::Phone(){}
Phone::Phone(string comp, string model,string version,float weight,float cameraf, float camerat,int mem_ram,int mem_rom,float price){
	Set_Company(comp);
	Set_Model(model);
	Set_Weight(weight);
	Set_Version(version);
...
char*  Phone::Get_PhoneNumber(){return phoneNumber;}
